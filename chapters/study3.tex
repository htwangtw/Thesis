...
\begin{wrapfigure}{o}{0.4\textwidth}    
    \vspace{-10pt}
    \centering
    \includegraphics[width=0.38\textwidth]{chapters/img/study3fig1.png}
	\caption{Grid search.} 
...
Component 8 put executive tasks against more automatic semantic retrieval. This component demonstrated a strong cognitive control ability.The functional connectivity pattern showed a disassociation among the transmodal system but high association the between the visual and the attention networks, and between the visual and the attention networks. 

Component 9 -  mental imagery vs. verbal fluency? The strong correlation between the default mode network and the visual system is associated with a 
...
component 7 (\textit{F}(13, 160) = 2.239, \textit{p} = .010, $\eta_{p}^{2}$ = .154) 
and 12 (\textit{F}(13, 160) = 1.946, \textit{p} = .029, $\eta_{p}^{2}$ = .137). There was only one significant univeriate effect after Bonferroni correction, 
...
(\textit{F}(13, 160) = 1.924, \textit{p} = .031, $\eta_{p}^{2}$ = .135)
...
component 7 
(\textit{F}(13, 160) = 2.192, \textit{p} = .012, $\eta_{p}^{2}$ = .151)
and component 12
(\textit{F}(13, 160) = 2.312, \textit{p} = .008, $\eta_{p}^{2}$ = .158). There was only significant univeriate effect in the model for component 7 after Bonferroni correction. 
